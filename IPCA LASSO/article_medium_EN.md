...
| Model | RMSE |
|---|---:|
| Elastic Net (BIC) | 0.320 |
| ARMA (BIC) | 0.320 |
| LASSO (BIC) | 0.321 |
| Focus (median) | 0.341 |
| Random Walk | 0.362 |
| 12-month average (AO) | 0.365 |
| Seasonal naïve | 0.471 |
...
At h = 1, LASSO (RMSE 0.321) beats the Random Walk (0.362) by about **11%**, and also edges the Focus median (0.341). Elastic Net and ARMA are essentially tied with LASSO at the top. The more important point, though, is not to look only at the point difference in RMSE.

The Diebold–Mariano test asks whether the observed error differences are consistent with **equal predictive accuracy**, given its assumptions. The p-values at h = 1 are:

| Comparison (LASSO vs.) | p — one-sided | p — two-sided |
|---|---:|---:|
| Random Walk | ⟨DM_RW_P1⟩ | ⟨DM_RW_P2⟩ |
| Atkeson–Ohanian | ⟨DM_AO_P1⟩ | ⟨DM_AO_P2⟩ |
| Seasonal naïve | ⟨DM_SRW_P1⟩ | ⟨DM_SRW_P2⟩ |
| ARMA | ⟨DM_ARMA_P1⟩ | ⟨DM_ARMA_P2⟩ |
| Focus | ⟨DM_FOCUS_P1⟩ | ⟨DM_FOCUS_P2⟩ |

The correct interpretation is not "the probability the result is luck is x%." Where the difference against the Focus is small or not statistically significant, the honest reading is that the model is *competitive with the market consensus*, not that it decisively beats it.
